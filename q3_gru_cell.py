...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            self.U_r = tf.get_variable('U_r',[self.input_size,self._state_size],tf.float32,tf.contrib.layers.xavier_initializer())
            self.W_r = tf.get_variable('W_r',[self._state_size,self._state_size],tf.float32,tf.contrib.layers.xavier_initializer())
            self.b_r = tf.get_variable('b_r',[self._state_size],tf.float32,tf.constant_initializer(0.))
            self.U_z = tf.get_variable('U_z',[self.input_size,self._state_size],tf.float32,tf.contrib.layers.xavier_initializer())
            self.W_z = tf.get_variable('W_z',[self._state_size,self._state_size],tf.float32,tf.contrib.layers.xavier_initializer())
            self.b_z = tf.get_variable('b_z',[self._state_size],tf.float32,tf.constant_initializer(0.))
            self.U_o = tf.get_variable('U_o',[self.input_size,self._state_size],tf.float32,tf.contrib.layers.xavier_initializer())
            self.W_o = tf.get_variable('W_o',[self._state_size,self._state_size],tf.float32,tf.contrib.layers.xavier_initializer())
            self.b_o = tf.get_variable('b_o',[self._state_size],tf.float32,tf.constant_initializer(0.))
 
            z_t = tf.sigmoid(tf.matmul(inputs,self.U_z)+tf.matmul(state,self.W_z)+self.b_z)
            r_t = tf.sigmoid(tf.matmul(inputs,self.U_r)+tf.matmul(state,self.W_r)+self.b_r)
            o_t = tf.tanh(tf.matmul(inputs,self.U_o)+r_t*tf.matmul(state,self.W_o)+self.b_o)
