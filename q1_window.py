...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        for word_idx in range(len(sentence)):
             start_point = word_idx - window_size
             end_point = word_idx + window_size
            
             start_token = start * max(0,-start_point)
             end_token = end * max(0,end_point+1-len(sentence))

             target_ = sentence[max(0,start_point):end_point+1]
             target_words = [item for word in target_ for item in word]
             result = start_token + target_words + end_token
            
             windowed_data.append((result,labels[word_idx]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        lookup = tf.nn.embedding_lookup(emb,self.input_placeholder)
