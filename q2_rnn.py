...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        new_sent = sentence + [zero_vector]*diff
        new_labels = labels + [zero_label]*diff
        mask = [True]*len(labels) + [False]*diff
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(lookup,[-1,self.max_length,self.config.n_features*self.config.embed_size])
    ...
    def add_loss_op(self, preds):
        ...
        loss_ = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=preds,labels=self.labels_placeholder)
